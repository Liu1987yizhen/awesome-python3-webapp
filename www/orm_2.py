...
if __name__ == "__main__":  # 一个类自带前后都有双下划线的方法，在子类继承该类的时候，这些方法会自动调用，比如__init__
    ...
    # 创建实例
    async def test():
        await create_pool(loop=loop, host='localhost', port=3306, user='root', password='qbslyz04067891.', db='test')
    ...
    async def d():
        await create_pool(loop=loop, host='localhost', port=3306, user='root', password='qbslyz04067891.', db='test')
        # u = user(name='lyz', tel=1233445667, email='xxxx@emai.com')
        # u2 = user(name='Micheal', tel=1233445667, email='xxxx@emai.com')
        # u3 = user(name='Bob', tel=1233445667, email='xxxx@emai.com')
        # u4 = user(name='Sarah', tel=1233445667, email='xxxx@emai.com')
        # await u.save()
        # await u2.save()
        # await u3.save()
        # await u4.save()
